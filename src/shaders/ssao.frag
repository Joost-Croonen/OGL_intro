...
void main()
{
	 ...
	 for (int i = 0; i<kernelSize; i++){
		vec3 tangentSamplePos = samples[i];							//tangent
		vec3 samplePos = fragPos + TBN * tangentSamplePos * radius; //view 
		vec4 offset = projection * vec4(samplePos, 1.0);			//~clip
		offset.xyz /= offset.w;										//~clip
		offset.xyz = offset.xyz * 0.5 + 0.5;						// 0. to 1. clip
